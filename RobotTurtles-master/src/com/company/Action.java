...
//il faut bien placer les boutons et ajouter une phrase qui indique qu'il faut choisir (il vaut peut etre meixu mettre le plateau en background)
    ...
    public void actionPerformed(ActionEvent e){
        ...
        if (source == this.button2) {
            ...
            Completer_programme completer_programme = new Completer_programme(joueur);
            joueur = completer_programme.completer();
    ...
    public Joueur commence(int i){//每个回合玩家只能在以下三选一
        if (i ==2) {
            System.out.println("compléter le programme");
            Completer_programme completer_programme = new Completer_programme(joueur);//填充预执行列表
            joueur = completer_programme.completer();
            return joueur;
        }
        if (i==1) {
            Executer_programme exec = new Executer_programme(plateau,joueur,listjoueurs);//执行预执行列表
            //Executer_programme exec = new Executer_programme();
            exec.mouvement(joueur,nbr_joueurs);
            return joueur;
        }
            ...
            System.out.println("construire un mur");
            Construire_mur cons = new Construire_mur(listjoueurs,listjoyaus,nbr_joueurs);//建立一个障碍物
            cons.construre(joueur,plateau);
            return joueur;
        }
        return joueur;
