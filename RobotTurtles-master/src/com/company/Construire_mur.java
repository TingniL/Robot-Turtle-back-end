...
public class Construire_mur extends JFrame implements ActionListener {
    ...
    public void construre(Joueur joueur,Plateau plateau){//此处由于同学做的图形界面卡住，运行时会被跳过
        this.joueur = joueur;
        this.plateau = plateau;
        ArrayList<Obstacles> listobstacle = joueur.getListobstacles();
        this.listobstacles = listobstacle;
        System.out.println("Vous avez encore : ");//列出该玩家有的障碍物卡片
        for (int i=0;i<listobstacle.size();i++){
            System.out.println(joueur.getobstable(i));
        }
        ...
        this.button1 = new JButton("mur en pierre");//石头障碍墙
        this.button1.addActionListener(this);
        b1.add(this.button1);

        Box b2 = Box.createHorizontalBox();
        this.button2 = new JButton("mur en glace");//冰块障碍墙
    ...
    public void actionPerformed(ActionEvent e){
        ...
        if (source == this.button1) {
            ...
            System.out.println("quelle ligne ?");//行
            int emplacement_ligne = scanner.nextInt();
            System.out.println("quelle colonne ?");//列
            int emplacement_colonne = scanner.nextInt();
            MurPierre murPierre = new MurPierre();
            placesurplateau(murPierre,emplacement_ligne,emplacement_colonne);//放入棋盘中
    ...
    public void placesurplateau(Obstacles obstacle,int ligne,int colonne){
        ...
        if (bloquer(nbr_joueurs)){//判定是否把玩家/宝石围住
