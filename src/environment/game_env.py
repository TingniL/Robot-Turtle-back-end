...
class RobotTurtlesEnv(gym.Env):
    ...
    def step(self, action: dict) -> Tuple[dict, float, bool, bool, dict]:
        ...
        info = {
            ...
            'program_size': len(player.program),
    ...
    def _is_valid_wall_position(self, position: Tuple[int, int]) -> bool:
        ...
        if x < 0 or x >= 8 or y < 0 or y >= 8:
            return False
        if self.board[position] != 0:  # 位置已被占用
            return False
        return True
    ...
    def get_valid_actions(self) -> List[dict]:
        ...
        # 添加墙放置动作
        if player.stone_walls > 0 or player.ice_walls > 0:
            for x in range(8):
                for y in range(8):
